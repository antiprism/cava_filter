                            ...
                            double range, int framerate, int autosens)
{
  ...
  if (number_of_bars < 1 ||
      number_of_bars > CAVA_TREBLE_BUFFER_SIZE / channels) {
    fprintf(stderr,
  ...
  if (range < 1) {
    fprintf(stderr,
            "cava_init called with illegal range %f, must be positive value "
            "above 0",
            range);
    exit(1);
  }
  if (framerate < 1) {
    fprintf(stderr,
            "cava_init called with illegal framerate %d, must be positive "
            "value above 0",
            framerate);
    exit(1);
  }
  ...
  p->treble_multiplier =
      (double *)malloc(p->FFTtreblebufferSize * sizeof(double));
  ...
  for (int i = 0; i < p->FFTmidbufferSize; i++) {
    p->mid_multiplier[i] =
        0.5 * (1 - cos(2 * M_PI * i / (p->FFTmidbufferSize - 1)));
  }
  for (int i = 0; i < p->FFTtreblebufferSize; i++) {
    p->treble_multiplier[i] =
        0.5 * (1 - cos(2 * M_PI * i / (p->FFTtreblebufferSize - 1)));
  }
  ...
  p->p_mid_l = fftw_plan_dft_r2c_1d(p->FFTmidbufferSize, p->in_mid_l,
                                    p->out_mid_l, FFTW_MEASURE);
  ...
  p->p_treble_l = fftw_plan_dft_r2c_1d(p->FFTtreblebufferSize, p->in_treble_l,
                                       p->out_treble_l, FFTW_MEASURE);
  ...
  memset(p->out_treble_l, 0,
         (p->FFTtreblebufferSize / 2 + 1) * sizeof(fftw_complex));
  if (p->audio_channels == 2) {
    ...
    p->p_mid_r = fftw_plan_dft_r2c_1d(p->FFTmidbufferSize, p->in_mid_r,
                                      p->out_mid_r, FFTW_MEASURE);
    ...
    memset(p->out_bass_r, 0,
           (p->FFTbassbufferSize / 2 + 1) * sizeof(fftw_complex));
    memset(p->out_mid_r, 0,
           (p->FFTmidbufferSize / 2 + 1) * sizeof(fftw_complex));
    memset(p->out_treble_r, 0,
           (p->FFTtreblebufferSize / 2 + 1) * sizeof(fftw_complex));
  }
  ...
  for (int n = 0; n < p->number_of_bars + 1; n++) {
    ...
    // the EQ is used to "normalize" them by dividing with this verry huge
    // number
    p->eq[n] /= pow(2, 20);
    ...
    if (p->cut_off_frequency[n] < bass_cut_off) {
      ...
      bar_buffer[n] = 3;
      p->FFTbuffer_lower_cut_off[n] =
          relative_cut_off[n] * (p->FFTtreblebufferSize / 2);
      first_treble_bar++;
                  ...
                  struct cava_plan *p)
{
  ...
  for (int n = 0; n < p->number_of_bars * p->audio_channels; n++) {
    ...
    if (cava_out[n] < p->prev_cava_out[n]) {
      cava_out[n] =
          p->cava_peak[n] - (p->g * p->cava_fall[n] * p->cava_fall[n]);
      if (cava_out[n] < 0)
        cava_out[n] = 0;
      p->cava_fall[n]++;
    }
